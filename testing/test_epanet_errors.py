...
from oopnet.simulators.simulation_errors import EPANETSimulationError
...
class BlankSimulationErrorTest(unittest.TestCase):
    ...
    def test_not_enough_nodes(self):
        from oopnet.simulators.simulation_errors import NotEnoughNodesError
        self._check_exception_raised(NotEnoughNodesError)

    def test_not_enough_sources(self):
        from oopnet.simulators.simulation_errors import NotEnoughSourcesError
...
class ExistingModelTest(unittest.TestCase):
    ...
    def test_node_not_connected(self):
        from oopnet.simulators.simulation_errors import UnconnectedNodeError
    ...
    def test_illegal_node_property(self):
        from oopnet.simulators.simulation_errors import IllegalNodePropertyError
        t = get_tank(self.network, 'T-1')
        t.diameter = -100
        self._check_exception_raised(IllegalNodePropertyError)

    def test_illegal_link_property(self):
        from oopnet.simulators.simulation_errors import IllegalLinkPropertyError
        j = get_pipe(self.network, 'P-1')
        j.diameter = -100
        self._check_exception_raised(IllegalLinkPropertyError)

    def test_invalid_curve(self):
        from oopnet.simulators.simulation_errors import InvalidPumpError
    ...
    def test_too_many_characters(self):
        from oopnet.simulators.simulation_errors import InvalidIDError
        j = get_pipe(self.network, 'P-1')
        j.id = 50 * 'a'
