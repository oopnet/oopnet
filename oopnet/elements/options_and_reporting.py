...
class Options:
    ...
    hydraulics: Optional[
        tuple[str, str]
    ] = None  # = Either(None, List('USE', Str), List('SAVE', Str))
    quality: Union[
        str, tuple[str, str], tuple[str, str, str]
    ] = "NONE"  # = Any #Either(None, Enum('NONE', 'CHEMICAL', 'AGE'), List('TRACE', Instance(Node)), List('CHEMICAL', Str, Str))
    viscosity: float = 1.0
...
class Times:
    ...
    qualitytimestep: Optional[
        datetime.timedelta
    ] = None  # todo: correct qualitytimestep and ruletimestep
...
class Reportparameter:
    ...
    elevation: Union[
        str, tuple[str, float]
    ] = "NO"  # = Either(None, Enum('YES', 'NO'), List('BELOW', Float), List('ABOVE', Float))
    demand: Union[
        str, tuple[str, float]
    ] = "YES"  # = Either(None, Enum('YES', 'NO'), List('BELOW', Float), List('ABOVE', Float))
    head: Union[
        str, tuple[str, float]
    ] = "YES"  # = Either(None, Enum('YES', 'NO'), List('BELOW', Float), List('ABOVE', Float))
    pressure: Union[
...
class Report:
    ...
    nodes: Union[
        str, Node, list[Node]
    ] = "ALL"  # = Either(None, Enum('NONE', 'ALL'), Instance(Node), List(Instance(Node)))
    links: Union[
        str, Link, list[Link]
