...
from oopnet.utils.getters.get_by_id import get_node, get_tank, get_pattern, get_pipe
...
def read_reaction(network: Network, block: list):
    ...
    for vals in block:
        ...
        if vals[0] == "ORDER":
            ...
            t = get_tank(network, vals[1])
            t.reactiontank = float(vals[2])
            if r.tank is None:
                r.tank = [t]
            else:
                r.tank.append(t)
