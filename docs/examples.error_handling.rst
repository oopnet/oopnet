...
This examples discusses handling errors raised by EPANET when simulating a model. You can find a list of all errors
EPANET might raise in the [EPANET docs](https://epanet22.readthedocs.io/en/latest/back_matter.html#error-messages).
...
If we were to run the simulation now, an :class:`OOPNET.EPANETSimulationError` would be raised by OOPNET. These exceptions act as
...
Since an :class:`OOPNET.EPANETSimulationError` is also a container, we can check for specific errors. You can check either be providing
...
is faulty and OOPNET also outputs these details.
